...
- Arama tarafından döndürülen sonuçların tahmini toplam boyutu 2 TB'ı aşarsa, raporları dışarı aktarma işlemi başarısız olur. Raporları başarıyla dışarı aktarmak için kapsamı daraltmaya çalışın ve sonuçların tahmini boyutu 2 TB'tan az olacak şekilde aramayı yeniden çalıştırın.
...
2. Arama açılır penceresinin altındaki **Eylemler** menüsünde **Raporu dışarı aktar'ı** seçin.
...
1. Uyumluluk portalındaki **İçerik arama** sayfasında **Dışarı Aktarmalar** sekmesini seçin
   ...
   Dışarı aktarma işlerinin listesini oluşturduğunuz dışarı aktarma işini gösterecek şekilde güncelleştirmek için **Yenile'yi** seçmeniz gerekebilir. Dışarı aktarma rapor işleri, arama adının sonuna **_ReportsOnly** ilgili aramayla aynı ada sahiptir.
  
2. 1. Adımda oluşturduğunuz dışarı aktarma işini seçin.

3. **Dışarı aktarma anahtarı altındaki Raporu dışarı** **aktar** açılır sayfasında **Panoya kopyala'yı** seçin. Arama sonuçlarını indirmek için 6. adımda bu anahtarı kullanırsınız.
...
6. **eBulma Dışarı Aktarma Aracı'nda** aşağıdakileri yapın:
   ...
   2. Arama raporu dosyalarını indirmek istediğiniz konumu belirtmek için **Gözat'ı** seçin.
...
- **Bildirim:** Arama sonuçlarına dahil edilen her öğe hakkında bilgi içeren bir bildirim dosyası (XML biçiminde). Yinelenenleri kaldırma seçeneğini etkinleştirdiyseniz, yinelenen iletiler bildirim dosyasına dahil değildir.
