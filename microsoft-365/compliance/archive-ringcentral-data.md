...
4. Bağlayıcı, [3. Adımda](#step-3-map-users-and-complete-the-connector-setup) açıklandığı gibi otomatik kullanıcı eşlemesinin *Email* özelliğinin değerini kullanarak dönüştürülen öğeleri belirli kullanıcıların posta kutularına aktarır. Kullanıcı posta kutularında Gelen Kutusu klasöründe **RingCentral** adlı bir alt klasör oluşturulur ve öğeler bu klasöre aktarılır. Bağlayıcı, *Email* özelliğinin değerini kullanarak öğelerin hangi posta kutusuna aktarılacağını belirler. Her RingCentral öğesi, öğenin her katılımcısının e-posta adresiyle doldurulmuş olan bu özelliği içerir.
...
1. <https://compliance.microsoft.com> Adresine gidin ve **Veri bağlayıcıları** > **RingCentral'ı** seçin.

2. **RingCentral** ürün açıklaması sayfasında **Bağlayıcı ekle'yi** seçin.
...
2. **Bağlayıcılar** sekmesini ve ardından **RingCentral** bağlayıcısını seçerek bağlayıcı hakkındaki özellikleri ve bilgileri içeren açılır sayfayı görüntüleyin.
