...
4. Kullanıcı posta kutularında **InvestEdge DataParser** adlı Gelen Kutusu klasöründe bir alt klasör oluşturulur ve InvestEdge öğeleri bu klasöre aktarılır. Bağlayıcı, *Email* özelliğinin değerini kullanarak öğelerin hangi posta kutusuna aktarılacağını belirler. Her InvestEdge öğesi, her katılımcının e-posta adresiyle doldurulmuş olan bu özelliği içerir.
...
1. <https://compliance.microsoft.com> Adresine gidin ve **Veri bağlayıcıları** > **InvestEdge DataParser'ı** seçin.

2. **InvestEdge DataParser** ürün açıklaması sayfasında **Bağlayıcı ekle'yi** seçin.
...
2. **Bağlayıcılar** sekmesini seçin ve ardından bağlayıcının özelliklerini ve bilgilerini içeren açılır sayfayı görüntülemek için oluşturduğunuz InvestEdge DataParser bağlayıcısını seçin.
